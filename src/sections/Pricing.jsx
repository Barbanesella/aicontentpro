...
const PLANS = [
  {
    ...
    features: ['40 AI content generations/mo','3 brand voice profiles','SEO articles, posts, newsletters','3 team seats (not per-seat)','SEO optimisation included','Email support'],
  ...
  },
  {
    ...
    features: ['200 AI content generations/mo','Unlimited brand voice profiles','All content types','10 team seats (not per-seat)','SEO + GEO optimisation','REST API (10k req/mo)','Priority support'],
    compare: 'vs Writer.com: $29/seat x 10 = $290 + no brand voice',
    highlight: true,
    badge: 'Most popular',
  },
  {
    ...
    features: ['Unlimited generations','Unlimited seats & voices','Full API + webhooks','Analytics dashboard','SSO & custom contracts','Dedicated account manager'],
  ...
  const payWithPayeer = async (plan) => {
    ...
    try {
      ...
      const res = await fetch('/api/payeer', {
        ...
        body: JSON.stringify({ amount: displayAmount.toFixed(2), plan: plan.name.toLowerCase(), email: userEmail }),
      })
    ...
    try {
      const res = await fetch('https://api.nowpayments.io/v1/invoice', {
        ...
        body: JSON.stringify({
          ...
          ipn_callback_url: 'https://aicontentpro.app/api/payment-webhook',
    ...
    <section id="pricing" style={s.section}>
      <div style={s.inner}>
        ...
        <div style={s.earlyBanner}>
          Early access: first 20 teams get 50% off forever + a personal onboarding call.
          ...
          {PLANS.map(function(plan) {
            return (
              <div key={plan.name} style={s.card(plan.highlight)}>
                  ...
                  {plan.features.map(function(f) {
                    return (
                      <li key={f} style={s.feature(plan.highlight)}>
                        <span style={s.check(plan.highlight)}>v</span>
                        {f}
                      </li>
                    )
                  })}
                ...
                <div style={s.ctaRow}>
                  <button style={s.cta(plan.highlight)} onClick={() => window.location.href = '/signup'}>
                    Start free trial
                  </button>
                  <button style={s.cryptoBtn(plan.highlight)} onClick={() => openCryptoModal(plan)}>
                    Pay with USDT
                  </button>
                  <button style={s.cryptoBtn(plan.highlight)} onClick={() => payWithPayeer(plan)} disabled={payeerLoading}>
                    Pay with Payeer
                  </button>
                </div>
      ...
      {modal && (
        <div style={s.modal} onClick={() => setModal(null)}>
          <div style={s.modalCard} onClick={function(e) { e.stopPropagation() }}>
            ...
            <div style={s.modalSub}>AI Content Pro - {modal.plan.name} Plan</div>
            <div style={s.modalAmount}>
              <span style={s.modalAmountLabel}>Total</span>
              <span style={s.modalAmountNum}>${modal.displayAmount}</span>
            </div>
            <div style={{ background: 'var(--paper-2)', border: '1px solid var(--paper-3)', borderRadius: '10px', padding: '12px 14px', marginBottom: '20px', fontSize: '12px', color: 'var(--ink-3)', lineHeight: 1.6 }}>
              Exchange fee already included. You pay exactly what you see.
            </div>
            <div style={{ marginBottom: '16px' }}>
              <label style={{ fontSize: '13px', fontWeight: 500, color: 'var(--ink-2)', display: 'block', marginBottom: '6px' }}>Your email</label>
              <input type="email" placeholder="you@company.com" value={email} onChange={function(e) { setEmail(e.target.value) }} style={{ width: '100%', padding: '11px 14px', border: '1.5px solid var(--paper-3)', borderRadius: '10px', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
            </div>
            ...
            <p style={s.cryptoNote}>USDT TRC-20 network. Powered by NOWPayments.</p>
          </div>
        </div>
      )}
    </section>
